		...
		<div class="pos_con">
			<div class="inner" v-if="isMultiColumn">
				<template v-for="(i,index) in getDataCur.posCom" class="g-cf">
					<div v-if="i.isShow" :itemMdata="i" :posCom="'posCom'" v-on:recomponent="recomponent" @moverModule="moverModule" @isFuncur="isFuncur" :index="index" :isfontFocus="fontFocus" :is="i.tName"></div>
				</template>
			</div>
		</div>
		<div :class="['conect_inner',{'warning':isConLeave}]" style="min-height: 500px; border: 1px solid #fff;">
			<div class="con_innd">
				<vue-context-menu :contextMenuData="contextMenuData" @delAllModule = "delAllModule()" @setAllModule="setAllModule()">
				</vue-context-menu>
				<template v-for="(i,tIndex) in getDataCur.stream" class="g-cf in_outer" :style="i.conStyle">
					<!--自由容器-->
					<div :data-index="tIndex" v-if="i.tName=='freeContainer'&&i.isShow" @dragover="onStreamOver($event,i)" @dragenter="onStreamEnter($event,i)" @drop.stop='inStreamDrop($event,i)' @dragleave="onStreamLeave($event,i)" :class="['in_inner freeContainer',{'enter':i.isStreamColumn}]" :style="[i.zinStyle,i.conStyle]">
					...
					<!--多列展示-->
					<div :data-index="tIndex" v-if="i.tName=='multiColumnContainer'&&i.isShow" :class="['in_inner multi_con g-cf', {'enter':i.isStreamColumn}]" :style="i.zinStyle">
						<div class="activ_con"><span @click="setUpDown(i,-1)" v-show="i.isShowUp" class="up_icon"></span><span @click="setUpDown(i,1)" v-show="i.isShowDown" class="down_icon"></span></div>
						<div class="multi_edit f_hv">
					...
					<!--标签展示-->
					<div :data-index="tIndex" @contextmenu="showMenu(i)" v-if="i.tName=='labelContainer'&&i.isShow" class="columnLayout tabContainer" :style="i.zinStyle" :id="'labelContainerId'+tIndex">
					...
					<!--通栏排版-->
					<div :data-index="tIndex" @contextmenu="showMenu(i)" :style="i.zinStyle" v-if="i.tName=='columnLayout'&&i.isShow" :class="['columnLayout', {'enter':i.isStreamColumn}]">
...
<script>
	...
	export default {
		...
		created() {
			...
			this.getDataCur.stream = allSite;
		...
		methods: {
			...
			showMenu(item) {
				...
				event.preventDefault();
				//console.log(this.contextMenuData.menulists[0].btnName = "管理全局模块");
				//console.log(this.contextMenuData.menulists[0].fnHandler = "managementGlobalModule");
				var x = event.clientX;
				var y = event.clientY;
			...
			managementGlobalModule() {
				console.log("ms");
				window.open(window.location.origin + '/pc/allsite');
			},
			delAllModule(e) {
				//console.log("ssss qq");
				/*var obj = {
				...
				this.delModule(this.transferIndex);
				var stream = this.getDataCur.stream;
				if(stream.length) {
					localStorage.setItem(this.param.id + this.param.type + "itemAllSite", JSON.stringify(stream));
				} else {
					localStorage.removeItem(this.param.id + this.param.type + "itemAllSite", JSON.stringify(stream));
				}
			},
			setAllModule(e) {
