<template>
	<vue-draggable-resizable :x="itemMdata.parentStyle.left" :y="itemMdata.parentStyle.top" :z="itemMdata.parentStyle.zIndex" :w="itemMdata.parentStyle.width" :h="itemMdata.parentStyle.height" :parent="itemMdata.parent" :active.sync="itemMdata.parentStyle.active" v-on:dragging="onDrag" :minw="1" :minh="1" v-on:resizing="onResize" :drag-cancel="'.edit_con'">
		<div :class="['articleScrap hove_con',{hover:isHover}]" @mouseenter="hoverFriendLink($event)" @mouseleave="leaveFriendLink($event)">
			<p class="edit_con" style="z-index:100;">
				<span @click="isFuncur(itemMdata)">编辑友情链接</span>
			...
			<div class="detail" @contextmenu="showMenu" :id="friendLinkId">
	...
	export default {
		...
		data() {
			return {
				...
				friendLinkId:"friendLinkId" + this.index,
				contextMenuData: {
		...
		mounted(){
			hoverAction.friendLinkHover(this.friendLinkId,this.itemMdata)
		},
		methods: {
			/*
			friendLinkOver(){
				$('#'+this.friendLinkId).find('li .text').css({
					"backgroundColor":this.itemMdata.timeStyle.backgroundColor,
				})
				$('#'+this.friendLinkId).find('li .text .title').css({
					"color":this.itemMdata.timeStyle.colorT,
				})
				$('#'+this.friendLinkId).find('li .text .describe').css({
					"color":this.itemMdata.timeStyle.colorD,
				})
			},
			friendLinkOut(){
				$('#'+this.friendLinkId).find('li .text').css({
					"backgroundColor":this.itemMdata.textStyle.backgroundColor,
					
				})
				$('#'+this.friendLinkId).find('li .text .title').css({
					"color":this.itemMdata.titleStyle.color,
					"color":this.itemMdata.describeStyle.color,
				})
				$('#'+this.friendLinkId).find('li .text .describe').css({
					"color":this.itemMdata.describeStyle.color,
				})
