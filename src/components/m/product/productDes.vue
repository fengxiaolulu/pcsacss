<template>
	<vue-draggable-resizable :x="itemMdata.parentStyle.left" :y="itemMdata.parentStyle.top" :w="itemMdata.parentStyle.width" :h="itemMdata.parentStyle.height" v-on:dragging="onDrag" :parent="itemMdata.parent" :active.sync="itemMdata.parentStyle.active" v-on:resizing="onResize" :drag-cancel="'.edit_con'">
		<div :class="['productDes hove_con',{hover:isHover}]" @mouseenter="hoverProductDes($event)" @mouseleave="leaveProductDes($event)">
			<p class="edit_con" style="z-index:100;">
				...
				<!--<span class="icon el-icon-edit" title="编辑产品"  @click="isProEd(itemMdata)"></span>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			//console.log(this.itemMdata.sibleItem[0])
			//百度分享
			let _this = this;
		...
		methods: {
			pre() {
				console.log(index)
				index++;
				if(this.$refs.conImgOne.length - index > 3) {
					moveLeft = -index * this.$refs.conImgOne[0].width - 10 * index;
					$(this.$refs.moveImgOne).css("left", moveLeft + 'px')
				}
				if(this.$refs.conImgTwo.length - index > 3) {
					moveLeft = -index * this.$refs.conImgTwo[0].width - 10 * index;
					$(this.$refs.moveImgTwo).css("left", moveLeft + 'px')
				}
				if(this.$refs.conImgThree.length - index > 3) {
					moveLeft = -index * this.$refs.conImgThree[0].width - 10 * index;
					$(this.$refs.moveImgThree).css("left", moveLeft + 'px')
				}
				if(this.$refs.conImgFive.length - index > 3) {
					moveTop = -index * this.$refs.conImgFive[0].height - 10 * index;
					$(this.$refs.moveImgFive).css("top", moveTop + 'px')
				}
			},
			next() {

				if(moveLeft == 0) {
					$(this.$refs.moveImgOne).css("left", 0)
					$(this.$refs.moveImgTwo).css("left", 0)
					$(this.$refs.moveImgThree).css("left", 0)
				}
				if(moveLeft < 0) {
					index--;
					moveLeft = -index * this.$refs.conImgOne[0].width - 10 * index;
			...
			nextFive() {
				if(moveTop == 0) {
					$(this.$refs.moveImgFive).css("top", 0)
				}
				if(moveTop < 0) {
					index--;
					console.log(index)
					moveTop = -index * this.$refs.conImgOne[0].height - 10 * index;
					$(this.$refs.moveImgFive).css("top", moveTop + 'px')
