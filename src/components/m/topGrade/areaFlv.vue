<template>
	<vue-draggable-resizable :x="itemMdata.parentStyle.left" :y="itemMdata.parentStyle.top" :z="itemMdata.parentStyle.zIndex" :w="itemMdata.parentStyle.width" :h="itemMdata.parentStyle.height" v-on:dragging="onDrag" :parent="itemMdata.parent" :active.sync="itemMdata.parentStyle.active" v-on:resizing="onResize" :drag-cancel="'.edit_con'">
		<div :class="['areaFlv hove_con',{hover:isHover}]" @mouseenter="hoverAreaFlv($event)" @mouseleave="leaveAreaFlv($event)" @contextmenu="showMenu">
			<p class="edit_con" style="z-index:100;">
				<span @click="isFuncur(itemMdata)">编辑在线视频</span>
			...
			<div class="detail">
				<ul>
					<li v-for="(table,index) in itemMdata.areaVideoTable" :style="{float:itemMdata.float,width:itemMdata.width}">
						<p>{{index}}.{{table.file_name}}<span v-show="itemMdata.size">{{table.file_size}}</span></p>
						<button>点击下载</button>
						<video controls="" :autoplay="itemMdata.autoplay" :style="itemMdata.videoStyle">
							<source :src="itemMdata.videoSrc" type="video/mp4">
						</video>
					</li>
				</ul>
				<!--分页-->
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData() {
				var v = this;
				api.get('/api/front/videoList?companyid=' + this.company_id + '&type=' + this.type, true, {}, function(data) {
					//var result = JSON.stringify(data); //json对象转成字符串
					v.itemMdata.areaVideoTable = data.data;
